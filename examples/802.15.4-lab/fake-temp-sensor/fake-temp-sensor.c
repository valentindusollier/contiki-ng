...
#define INTERPUBLISH_SPACE (2 * CLOCK_SECOND)
#define DEFAULT_PUBLISH_INTERVAL (120 * CLOCK_SECOND)
/*---------------------------------------------------------------------------*/
...
static void publish_temperature(void *p) {
  publish(temperature_topic, temperature);
}
static void publish_humidity(void *p) {
...
PROCESS_THREAD(mqtt_client_process, ev, data) {
  ...
  while (1) {
    ...
    case STATE_PUBLISHING: /* Publishing */
      leds_on(MQTT_PUBLISHING_LED);
      ctimer_set(&ct, WAITING_INTERVAL, status_led_off, NULL);
      int temperature_decimal = (int)(4 + 2 * sin(2 * 3.14 * clock_seconds() / (45 * 60)));
      int humidity_integer = (int)(59 + 7 * sin(2 * 3.14 * clock_seconds() / (2 * 60 * 60)));
      snprintf(temperature, 20, "20.%d", temperature_decimal);
      snprintf(humidity, 20, "%d.0", humidity_integer);
      LOG_INFO("Temperature: %s, Humidity: %s\n", temperature, humidity);
      LOG_DBG("Publishing\n");
      publish_temperature(NULL);
      ctimer_set(&ips_ct, INTERPUBLISH_SPACE, publish_humidity, NULL);
